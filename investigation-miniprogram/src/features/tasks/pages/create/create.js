...
Page({
    ...
    async loadLists(preselectedListId = '') {
        ...
        try {
            ...
            tasks_2.tasksStore.setState({ lists, activeListId: currentList.id || null });
        }
        catch (error) {
            console.error('Failed to load lists:', error);
            this.setData({
                isLoading: false,
                errorMessage: error instanceof Error ? error.message : '加载清单失败'
            });
    ...
    async onSave() {
        ...
        this.setData({ isLoading: true, errorMessage: '' });
        try {
            const newTask = await (0, tasks_1.createTask)({
            ...
            tasks_2.tasksStore.setState((prev) => ({
                ...
                tasksByListId: this.data.listId
                    ? Object.assign(Object.assign({}, prev.tasksByListId), { [this.data.listId]: [...(prev.tasksByListId[this.data.listId] || []), newTask] })
                    : prev.tasksByListId
            }));
