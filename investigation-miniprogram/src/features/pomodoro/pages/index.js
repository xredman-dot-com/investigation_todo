...
const pomodoro_1 = require("../services");
const pomodoro_2 = require("../../../stores/pomodoro");
Page({
    data: {
        sessions: [],
        duration: "25",
    ...
    async fetchSessions(forceRefresh = false) {
        const cached = pomodoro_2.pomodoroStore.getState();
        if (!forceRefresh && cached.sessions.length) {
            this.setData({ sessions: cached.sessions });
        }
        pomodoro_2.pomodoroStore.setState({ loading: true, error: null });
        this.setData({ isLoading: true, errorMessage: "" });
        try {
            const sessions = await (0, pomodoro_1.listPomodoroSessions)();
            pomodoro_2.pomodoroStore.setState({ sessions, loading: false, error: null });
            this.setData({ sessions, isLoading: false, errorMessage: "" });
        }
        catch (error) {
            pomodoro_2.pomodoroStore.setState({
                loading: false,
